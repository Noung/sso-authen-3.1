...
class AuthController
{
    public function login()
    {
        try {
            ...
            // Log admin OIDC login initiated
            $this->logAdminAction(
                'system',
                'admin_oidc_login_initiated',
                'authentication',
                'admin-panel',
    ...
    public function callback()
    {
        try {
            ...
            if ($this->isAdminUser($userInfo['email'])) {
                ...
                $this->logAdminAction($userInfo['email'], 'admin_login', 'authentication', 'admin-panel', $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1', $_SERVER['HTTP_USER_AGENT'] ?? 'Unknown');
                // Log OIDC authentication success
                $this->logAdminAction($userInfo['email'], 'admin_oidc_auth_success', 'authentication', 'admin-panel', $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1', $_SERVER['HTTP_USER_AGENT'] ?? 'Unknown');
            ...
            // Log OIDC authentication failure
            $this->logAdminAction(
                'system',
                'admin_oidc_auth_failed',
                'authentication',
                'admin-panel',
                $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1',
                $_SERVER['HTTP_USER_AGENT'] ?? 'Unknown'
    ...
    private function isAdminUser($email)
    {
        try {
            ...
            if (
                $adminConfig['auth']['development']['enabled'] &&
